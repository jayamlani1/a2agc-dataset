WITH
  ...
  labels AS (
	...
	END as A,
	CASE
	  WHEN JUSTICE_INVOLVEMENT + HEALTH_DATA = 2 THEN "Jail + Hosp"
	  WHEN JUSTICE_INVOLVEMENT = 1 THEN "Jail"
	  WHEN HEALTH_DATA = 1 THEN "Hosp"
	  ELSE "∅"
	END as B,
	CASE
	  WHEN JUSTICE_INVOLVEMENT + HEALTH_DATA + OPIOID_PRESCRIPTION + OVERDOSE = 4 THEN "Rx + OD + Jail + Hosp"
	  WHEN JUSTICE_INVOLVEMENT + HEALTH_DATA + OVERDOSE = 3 THEN "OD + Jail + Hosp"
	  WHEN JUSTICE_INVOLVEMENT + HEALTH_DATA + OPIOID_PRESCRIPTION = 3 THEN "Rx + Jail + Hosp"
	  WHEN JUSTICE_INVOLVEMENT + OVERDOSE + OPIOID_PRESCRIPTION = 3 THEN "Rx + OD + Jail"
	  WHEN HEALTH_DATA + OVERDOSE + OPIOID_PRESCRIPTION = 3 THEN "Rx + OD + Hosp"
	  WHEN JUSTICE_INVOLVEMENT + HEALTH_DATA = 2 THEN "Jail + Hosp"
	  WHEN JUSTICE_INVOLVEMENT + OVERDOSE = 2 THEN "OD + Jail"
	  WHEN JUSTICE_INVOLVEMENT + OPIOID_PRESCRIPTION = 2 THEN "Rx + Jail"
	  WHEN HEALTH_DATA + OPIOID_PRESCRIPTION = 2 THEN "Rx + Hosp"
  	  WHEN OPIOID_PRESCRIPTION + OVERDOSE = 2 THEN "Rx + OD"
	  WHEN HEALTH_DATA + OVERDOSE = 2 THEN "OD + Hosp"
	  WHEN JUSTICE_INVOLVEMENT = 1 THEN "Jail"
	  WHEN OVERDOSE = 1 THEN "OD"
	  WHEN OPIOID_PRESCRIPTION = 1 THEN "Rx"
	  WHEN HEALTH_DATA = 1 THEN "Hosp"
