WITH
  ...
  CN_POJH2 AS (
    SELECT DOD, CASE_NUMBER, PERIOD, RANK, 'Opiod Prescriptions' AS Encounter_Type, OPIOID_PRESCRIPTIONS AS COUNT
      FROM CN_POJH
    UNION ALL
    SELECT DOD, CASE_NUMBER, PERIOD, RANK, 'Overdoses' AS Encounter_Type, OVERDOSES AS COUNT
      FROM CN_POJH
    UNION ALL
    SELECT DOD, CASE_NUMBER, PERIOD, RANK, 'Incarcerations' AS Encounter_Type, INCARCERATIONS AS COUNT
      FROM CN_POJH
    UNION ALL
    SELECT DOD, CASE_NUMBER, PERIOD, RANK, 'Health Encounters' AS Encounter_Type, HEALTH_ENCOUNTERS AS COUNT
      FROM CN_POJH
    UNION ALL
    SELECT DOD, CASE_NUMBER, PERIOD, RANK, 'All Types' AS Encounter_Type, ALL_TYPES AS COUNT
      FROM CN_POJH
  )
SELECT *
FROM CN_POJH2
